...
- **Stack** – React Native + Expo, Firebase (Auth, Firestore), Expo Location/Task Manager, Expo Notifications, OpenStreetMap + OSRM routing.
...
| Persona          | Highlights                                                                                                                                                                       |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Students**     | Unified login, real-time bus map with OpenStreetMap tiles, stop timeline/ETA, attendance history, and report submission to management or bus incharge.                           |
...
- `MapScreen` overlays OpenStreetMap tiles via `react-native-maps`.
- OSRM polylines generated by `utils/routePolylineConfig.js` show planned routes.
...
- **Polyline missing or straight line** – OSRM call failed; inspect Metro logs for `routeWarning` and verify network access to the routing service.
...
- Evaluate hosting private map/OSRM infrastructure if public endpoints become rate-limited.
...
- **Students** – Unified login, real-time bus view with OpenStreetMap tiles, stop timeline, attendance history, and report submission to either management or the assigned bus incharge.
...
- **Maps & Routing** via `react-native-maps`, OpenStreetMap tile overlays, and OSRM polyline generation (`utils/routePolylineConfig.js`).
